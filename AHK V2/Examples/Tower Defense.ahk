...
game := ShinsGameClass(0,0,1000,800,"Tower Defense by Spawnova",vsync)
...
game.data.spawner := spawnerClass(game)
...
game.data.towers := []
game.data.towers.push(tower1Class(game,820,300,0))
game.data.towers.push(tower2Class(game,920,300,0))
game.data.towers.push(tower3Class(game,820,400,0))
...
game.data.pathGuide := pathGuideClass(game)

; Create a placement marker for placing towers
game.data.placeMarker := placeMarkerClass(game)

game.data.speedControl := speedControlClass(game,800-138,800-74)
...
#HotIf WinActive("Tower Defense by Spawnova")
		...
		for k,v in this.game.data.enemies
		{
			...
			if (v.active and v.health > 0 and this.EnemyInRange(v,&dist)) {
				...
				if (this.img = "resources\images\proj1.png") {
					ps := 5
					loop ps {
						part := particleClass(this.game,this.x+random(-4,4),this.y+random(-4,4))
					...
					part := particleClass(this.game,this.x,this.y)
					part.SetImage("resources\images\hit.png")
					part.scaleX := part.scaleY := random(2.5,4.2)
					part.direction := this.angleRad + random(-0.3,0.3)
					part.speed := random(60,120)
		...
		if (this.spawning) {
			if (this.delay <= 0) {
				if (this.spawnCount > 0)
					this.game.data.enemies.push(enemyclass(this.game,this.spawns[this.waveIndex][this.spawnIndex].id))
				this.count++
	...
	Update() {
		...
		if (this.game.lbutton) {
			...
			if (this.game.data.money >= this.cost) {
				...
				if (this.id = 0)
					this.game.data.towers.push(tower1Class(this.game,this.x,this.y,1))
				else if (this.id = 1)
					this.game.data.towers.push(tower2Class(this.game,this.x,this.y,1))
				else if (this.id = 2)
					this.game.data.towers.push(tower3Class(this.game,this.x,this.y,1))
			}
			this.active := 0
		} else {
		...
		if (this.active) {
			...
			if (this.attackReset <= 0) {
				if (ci) {
					...
					part := particleClass(this.game,bx,by)
					...
					hit := particleClass(this.game,v.x,v.y)
		...
		if (this.active) {
			...
			if (this.attackReset <= 0) {
				if (ci) {
					...
					this.GetBarrelPosition(&bx,&by)
					this.attackReset := this.attacksPerSecond
					proj := projectileClass(this.game,bx,by,this.projImg)
					proj.speed := this.projSpeed
					proj.damage := this.damage
					proj.SetTargetAngle(this.angleRad)
					this.game.data.projectiles.push(proj)
					loop 3 {
						smoke := particleClass(this.game,bx+random(-5,5),by+random(-5,5))
			...
			if (this.attackReset <= 0) {
				if (ci) {
					...
					loop 8 {
						...
						smoke := particleClass(this.game,bx,by)
